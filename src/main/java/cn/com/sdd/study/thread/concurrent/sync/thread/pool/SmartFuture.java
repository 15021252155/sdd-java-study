...
/**
 * @ClassName SmartFuture
 * @Author suidd
 * @Description 异步Future的实现
 * 上一节我们已经提到，Future是用来执行任务的结果，JDK自带的Future实现FutureTask，只能同步等待结果，当get方法被调用的时候，当前线程就会被阻塞，一直到任务执行完成，或者一直等待到超时。
 * <p>
 * 在Netty中，提供了另外一种Future实现(ChannelFuture)，其是异步的，也就是说不需要同步等待执行结果，其可以在任务执行完成之后，回调用户指定的方法，以告诉我们任务的结果。
 * <p>
 * 当然，这里并不是会去分析Netty中这种ChannelFuture的实现源码，我们只是要实现一个类似功能的Future实现，在这里称之为SmartFuture。其支持同步等待返回结果，也支持异步通知结果。
 * <p>
 * 由于FutureTask已经提供了同步等待的功能，所以我们只需要让我们的SmartFuture继承FutureTask，再添加相关异步功能的方法即可
 * @Date 22:54 2020/5/5
 * @Version 1.0
 **/
public class SmartFuture<V> extends FutureTask<V> {//异步通知的listener
